...
public class EEmitterGroup : MonoBehaviour
{
    [System.Serializable]
    private class GroupedEmitter
    {
        public GroupedEmitter(EEmitter _emitter, EEmitterGroup _parent, float _zRotationOnStart, float _zRotationRandomOffset = 0, float _randomDelay = 0)
        {
            emitter = _emitter;
            Parent = _parent;
        ...
        public EEmitterGroup Parent { get; set; }
    ...
    {
        EEmitter[] newEmitters = gameObject.GetComponentsInChildren<EEmitter>();

        emitters = new List<GroupedEmitter>();
        foreach (EEmitter emitter in newEmitters)
        {
